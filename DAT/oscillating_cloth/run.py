...
# -------------------------
cam_name = "Camera"
# 3 cloth meshes - vertices evenly divided
mesh_names = ["initial_cloth1", "initial_cloth2", "initial_cloth3"]
...
while fileId < numFrames:
    ...
    # Load all vertices
    V_all = np.load(npy_path)
    
    # Split evenly among 3 meshes
    n_total = V_all.shape[0]
    n_per_mesh = n_total // 3
    
    # Update each mesh with its portion of vertices
    for i, obj in enumerate(mesh_objs):
        start_idx = i * n_per_mesh
        end_idx = (i + 1) * n_per_mesh if i < 2 else n_total  # Last mesh gets remainder
        V = V_all[start_idx:end_idx]
