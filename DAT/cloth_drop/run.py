...
# Generate mesh names dynamically based on pattern: initial_mesh_XXX_cloth_main_cloth_layerN
num_layers = args.numLayers
mesh_prefix = args.meshPrefix
mesh_names = [f"{mesh_prefix}_{i:03d}_cloth_main_cloth_layer{i}" for i in range(num_layers)]
print(f"Configured for {num_layers} cloth layers")
...
# Get vertex counts from actual meshes
mesh_vertex_counts = [len(obj.data.vertices) for obj in mesh_objs]
print(f"Mesh vertex counts: {mesh_vertex_counts}")
...
while fileId < numFrames:
    ...
    # Split based on actual mesh vertex counts
    start_idx = 0
    for i, obj in enumerate(mesh_objs):
        end_idx = start_idx + mesh_vertex_counts[i]
